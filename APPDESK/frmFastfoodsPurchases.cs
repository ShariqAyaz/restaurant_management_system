...
namespace APPDESK
{
    public partial class frmFastfoodsPurchases : MetroForm
    {
        ...
        getFastfoodTicket gt = new getFastfoodTicket();

        transactions.transactions tr = new transactions.transactions();
        int appid = 1029;
        public frmFastfoodsPurchases(int get_bid, int get_userid)
        {
            InitializeComponent();
            bid = get_bid;
            userid = get_userid;
        ...
        private void btnFetchData_Click(object sender, EventArgs e)
        {
            if (bid == 2)
            {
                ...
                try
                {
                    ...
                    reportDataSource2.Value = this.FFDT_FFPURCHASEBindingSource;
                    ...
                    this.fFDT_FFPURCHASE_TA.Fill(this.eSITERPDataSet._FFDT_FFPURCHASE, Convert.ToInt32(txtST_T.Text));
            ...
            {
                MessageBox.Show("There is no applicable operation for fastfood user. if you are seeing this window. Please discuss Mr. Shahzad");
            }
        }
        ...
        private void btnTransfer_Click(object sender, EventArgs e)
        {
            ...
            int tnumber = num.fastfood_transfer_no();
            //
            if (bid == 2)
            {
                try
                {
                    ...
                    while (rdrSeek.Read() == true)
                    {
                        try
                        {
                            ...
                            SqlCommand cmdIns = new SqlCommand("INSERT INTO[dbo].[FASTFOOD_PURCHASES] ([start_T],[end_T],[transfer_no],[prod_date],[GroupCode],[MID],[MenuItemName],[QTY],[LINE_AMT],[acc_id],[paid_amt],[b_entity_id],[uid])VALUES(" + st_t + "," + end_t + "," + tnumber + ",'" + dtpPdate.Value + "','" + (string)rdrSeek["GroupCode"] + "'," + (int)rdrSeek["MID"] + ",'" + (string)rdrSeek["MenuItemName"] + "'," + (decimal)rdrSeek["QTY"] + "," + (decimal)rdrSeek["LINE_AMT"] + "," + get_acc_id + "," + Convert.ToDecimal(txtbAmt.Text) + "," + bid + "," + userid + ")", conIns);
                            conIns.Open();
                ...
                try
                {
                    ...
                    SqlCommand cmdUPD = new SqlCommand("update numerator set FASTFOOD_PURCHASE=" + (end_t + 1), conUPD);
                    conUPD.Open();
                ...
                tr.fastfood_purchase_pay_account_transaction(tnumber, userid, bid, dtpPdate.Value, get_acc_id, Convert.ToDecimal(txtbAmt.Text));
                MessageBox.Show("inserted");
            }
            else if (bid == 1)
            {
                MessageBox.Show("This is dedicate report/Option for restaurant user. Pls Discuss with Information Technology. Or Mr. Shahzad");
            }
        }
        int get_acc_id;
